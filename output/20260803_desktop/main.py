...
def create_comparison_plot(dataset, model_type, strategy, func, sel_mode, index, org_data, ae_data, model, current_date, lim, true_label=None, output_dir=None, answer_records=None):
    """クイズ用に正常データ/敵対的サンプルを見分けられない形で2枚保存する。
    どちらがA/Bになるかはサンプルごとにランダムに決め、画像側には色分け・タイトル・
    分類結果テキストなど答えにつながる情報を一切載せない。
    正解（どちらが敵対的か、誤分類に成功したか等）は answer_records に記録し、
    出題者だけが見る別ファイル（answer key）として保存する。"""
    try:
        ...
        try:
            ...
            org_class = int(np.argmax(org_pred[0]))
            ae_class = int(np.argmax(ae_pred[0]))

            if true_label is not None:
                true_label_int = int(true_label)
                misclassified = (ae_class != true_label_int)
            else:
                true_label_int = None
                misclassified = (ae_class != org_class)
        except Exception as e:
        ...
        # 出題用の2枚は同一フォルダに、色・タイトル・凡例など答えのヒントになる
        # 情報を一切含めずに保存する。org/ae フォルダ分けもしない（フォルダ名で
        # わかってしまうため）。
        quiz_dir = os.path.join(base_plot_dir, dataset, "quiz")
        if not os.path.exists(quiz_dir):
            os.makedirs(quiz_dir)

        base_filename = f'{current_date}_{dataset}_{model_type}_{strategy}_{func}_{sel_mode}_lim{lim}_{index+1}'

        # A/Bどちらが正常データになるかをサンプルごとにランダムに決定
        if random.random() < 0.5:
            slot_data = {"A": (org_row, "org"), "B": (ae_row, "ae")}
        else:
            slot_data = {"A": (ae_row, "ae"), "B": (org_row, "org")}

        filenames = {}
        for slot, (row, _kind) in slot_data.items():
            plt.figure(figsize=(12, 6))
            x = np.arange(1, len(row) + 1)
            plt.plot(x, row, color='steelblue', linewidth=2)
            plt.title(f'{dataset} - Sample {index+1} - {slot}', fontsize=14, fontweight='bold')
            plt.xlabel('Time Steps', fontsize=12)
            plt.ylabel('Values', fontsize=12)
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            filename = f'{base_filename}_{slot}.png'
        ...
        adversarial_slot = "A" if slot_data["A"][1] == "ae" else "B"
        original_slot = "A" if slot_data["A"][1] == "org" else "B"
        ...
        if answer_records is not None:
            answer_records.append({
                ...
                "adversarial_slot": adversarial_slot,
...
def save_quiz_answer_key(answer_records, output_dir, dataset, model_type, strategy, func, sel_mode, lim):
    """出題者だけが参照する答え合わせ用ファイル。quiz/ フォルダには入れず、
    実行結果ディレクトリ直下に保存する（クイズ画像と一緒に配らないこと）。"""
    if not answer_records:
        return

    filename = f"answer_key_{dataset}_{model_type}_{strategy}_{func}_{sel_mode}_lim{lim}.xlsx"
    filepath = os.path.join(output_dir, filename)

    df = pd.DataFrame(answer_records)
    df.to_excel(filepath, index=False)
    print(f"答え合わせ用ファイル保存完了: {filepath}")
...
def make_fitness_batch(model, x_sample, label, alpha, K_max):
    ...
    def fitness_batch(population):
        ...
        return alpha * misclass + (1.0 - alpha) * npert # [-1,0]
...
def process_single_sample(index, x_sample, y_sample, model,
    ...
    generation_logs = []
    fitness_batch = make_fitness_batch(model, x_sample, label, alpha, K_max)
                ...
                try:
                    ...
                    for i in range(num_rows):
                        create_comparison_plot(
                            ...
                            output_dir=current_date_dir,
                            answer_records=quiz_answer_records
                        )
                        if (i + 1) % 10 == 0:
                            print(f"画像生成進捗: {i + 1}/{num_rows} 完了")

                    save_quiz_answer_key(quiz_answer_records, current_date_dir, dataset, model_type, strategy, func, sel_mode, lim)
